...
const config = {
  ...
  'location': {
    ...
    fallback: '/tmp/hsd-bench-blockstore'
  },
  'store': {
    value: true,
...
(async () => {
  ...
  for (const hash of hashes) {
    const start = process.hrtime();
    const raw = await store.readBlock(hash);
    const elapsed = process.hrtime(start);
